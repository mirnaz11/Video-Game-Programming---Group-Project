...
public class Car {
	...
	public void draw(Graphics g) {
		g.drawRect(x_Rect - Camera.x, y_Rect, 60, 20);
	...
	public void slide(SmallRamp ramp, int count) {
		...
		if (ramp.hasCollidedWith(this)) {// double adj = car.x_Rect -
											// ramp.distanceTo(car.x_Rect,
											// car.y_Rect);
		...
		/*
		 * else if(count == 0) { slide = false; if((x_Rect > ramp.x1 - 20))
		 * moveBackwardBy(5); }
		 */
	...
	public void projectileMotion() {
		if (y_Rect + 20 < 500) {
			...
			if (theta1 >= 5 && theta2 >= 5 && isFallingDown == false) {
				...
				System.out.println("flying1:                " + isFlying
						+ isFallingDown);
			...
			/*
			 * int dy = ramp.y1 - ramp.y2; //x = x0 + vix*t + ax * t^2 where ax
			 * =0 y_Rect = y_Rect + dy + (int)9.8/2;
			 */
	...
	public void upRamp(SmallRamp ramp) {
		double dist = ramp.distanceTo(x_Rect, y_Rect - 20);
		if ((SmallRamp.y2 < y_Rect + 20) && (SmallRamp.x2 > x_Rect)) {// double
																		// adj =
																		// car.x_Rect
																		// -
																		// ramp.distanceTo(car.x_Rect,
